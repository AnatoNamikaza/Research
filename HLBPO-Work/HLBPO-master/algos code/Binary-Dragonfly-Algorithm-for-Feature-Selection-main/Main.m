%-------------------------------------------------------------------%
%  Binary Dragonfly Algorithm (BDA) with KNN Imputation and Timing  %
%-------------------------------------------------------------------%
...
Acc_results_BDA = zeros(length(datasets), runs);
...
for i = 1:length(datasets)
    ...
    % Storage for curves
    Curves_BDA = zeros(runs, max_Iter);

    % Run Binary Dragonfly Algorithm multiple times
    for run = 1:runs
        ...
        Acc_BDA = jKNN(sFeat, label, HO); % Replace with your own accuracy calculation
        Acc_results_BDA(i, run) = Acc_BDA;

        Curves_BDA(run,:) = curve;

        % Save the results to a file
        results_folder_BDA = 'BDA_results/';
        if ~exist(results_folder_BDA, 'dir')
            mkdir(results_folder_BDA);
        end

        file_path_BDA = fullfile(results_folder_BDA, strcat(datasets{i}, '.txt'));
        fileID = fopen(file_path_BDA, 'a');
    ...
    [min_value, min_index] = min(Curves_BDA(:, end));

    % Plot BDA convergence curve
    fig_BDA = figure('Visible', 'off');
    plot(1:max_Iter, Curves_BDA(min_index,:));
    xlabel('Number of Iterations');
    ylabel('Fitness Value');
    title(['BDA Convergence Curve for ', datasets{i}]);
    grid on;

    img_folder_BDA = 'BDA_images/';
    if ~exist(img_folder_BDA, 'dir')
        mkdir(img_folder_BDA);
    end
    img_path_BDA = fullfile(img_folder_BDA, strcat(datasets{i}, '.png'));
    saveas(fig_BDA, img_path_BDA);
...
disp('Accuracy results for Binary Dragonfly Algorithm for each dataset:');
disp(Acc_results_BDA);
