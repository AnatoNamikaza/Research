%---------------------------------------------------------------------%
%  Sine Cosine Algorithm (SCA) with KNN Imputation and Timing         %
%---------------------------------------------------------------------%
...
for i = 1:length(datasets)
    ...
    % Run the SCA algorithm multiple times
    for run = 1:runs
        ...
        % Calculate the accuracy (Assuming you have a function similar to jKNN)
    ...
    title(['SCA Convergence Curve for ', datasets{i}]);
    grid on;

    % Save the convergence plot
    img_folder = 'SCA_images/';
