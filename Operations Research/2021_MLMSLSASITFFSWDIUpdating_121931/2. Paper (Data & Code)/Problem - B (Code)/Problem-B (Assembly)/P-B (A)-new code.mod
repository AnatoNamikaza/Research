...
main{
	...
	//for(var t in thisOplModel.macroperiods){
		//if(t==1){
			if (cplex.solve())
			{
			//var ofile = new IloOplOutputFile("E:/2. Research Work (PhD-MS&E)/0. My Research Work/2. MGLSPMS with forecast updates/2. Paper (Data & Code)/Problem - B (Code)/All Results (P-B)/TAS1C1D1.txt");
			...
			//thisOplModel.convertAllIntVars(); // Relax all the integer variables (Converting IP/MIP to LP)
			
//		for (var j in thisOplModel.products)
//			for(var t in thisOplModel.macroperiods) {
//	   			writeln("And then we freeze Inventory[1][0]",thisOplModel.inventory[j][t].UB=500); 
//				writeln("And then we freeze Inventory[1][0]",thisOplModel.inventory[j][t].LB=5);		
    //  }       	
       //thisOplModel.stagesetup[1][3][3].UB=1; 
      // thisOplModel.stagesetup[1][3][3].LB=1; 
           
//       cplex.solve(); 
//       writeln();
//       writeln("After Relaxing Integer Variables: ");
//       writeln();
//       thisOplModel.postProcess();   
...
execute { 
    ...
    for (var j=1; j<=10; j++)
    	for(var l=1; l<=3; l++)
    		for(var s=1; s<=12; s++)
    		writeln("Production Stage Setup for Proudtc[",j,"][",l,"][",s,"]=",stagesetup[j][l][s]); 			
    for (var i=1; i<=10; i++)
    	for (var j=1; j<=10; j++)
    		for(var l=1; l<=3; l++)
    			for(var s=1; s<=12; s++)
