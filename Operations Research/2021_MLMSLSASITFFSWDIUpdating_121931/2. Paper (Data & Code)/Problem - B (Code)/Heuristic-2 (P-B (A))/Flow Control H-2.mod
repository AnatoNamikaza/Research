 ...
 * Creation Date: May 28, 2017 at 1:36:56 AM
 *********************************************/
execute Timelimit {
		cplex.tilim=3600;
				 }			 
main {
	...
	writeln(); 
    writeln("Freeze the Production Setup & Production Quantity Variables which obtained from SSt Model"); 
    writeln(); 
	...
	for(var j in oplmodel4.products)
	  for(var l in oplmodel4.productionstages)
		...
		var ofile = new IloOplOutputFile("E:/2. Research Work (PhD-MS&E)/0. My Research Work/2. MGLSPMS with forecast updates/2. Paper (Data & Code)/Results (Problem B & D)/Heuristic-II Results (B & D)/Results P-B (Assembly)/Setup Profile-II/TAS2C5D9.txt");
